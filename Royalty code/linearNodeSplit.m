...
function [childrenBest, infoGainBest] = linearNodeSplit(minYInt, maxYInt, rootNode, numfunct) % Compute the best "y=mx+p" split node for the bag
    ...
    Grad1 = (randomSampYInt - min(rootNode(:,2))/0.001)*0.001./(-rootNode(rootNode(:,2) == min(rootNode(:,2)),1));
    Grad2 = (randomSampYInt - max(rootNode(:,2))/0.001)*0.001./(-rootNode(rootNode(:,2) == max(rootNode(:,2)),1));
    ...
    % calculate a random gradient in between the min and max possible
        randomSampGrad(m,:) = minGrad/0.001 + randperm(round((maxGrad-minGrad)/0.001),numfunct);
